...
sub render_align_bar {
  ...
  if($config->{'_parameters'}->{'splitflag'} > 0) {

    my $xc1  = ($config->{'_parameters'}->{'splitflag'} == 1) ? ($self->{'container'}->end - $global_start - 16/$pix_per_bp) : ($global_start + 6/$pix_per_bp);
    my $h1   = $yc - 2;
    my $colour1 = 'fc0000';
    my $base1 = $self->strand == 1 ? $h1 + 4 : $h1 - 5;

    $self->push($self->Poly({
        colour    => $colour1,
        absolutey => 1,
        points    => [
           $xc1 - 4/$pix_per_bp,$base1,
           $xc1 - 4/$pix_per_bp, $base1 + 8,
	   $xc1 + 4/$pix_per_bp, $base1+ 4
        ]
    }));
