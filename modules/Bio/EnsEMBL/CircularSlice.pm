...
sub centrepoint {
  my $self = shift;
  my ($s, $e, $length) = ($self->{'start'}, $self->{'end'}, $self->{'seq_region_length'});
  if ($s < $e) {
      return ($s + $e)/2;
  } else {
      my $r1 = $length - $s;
...
sub length {
  ...
  if ($self->{'start'} < $self->{'end'}) {
      ...
      my $ln = $r1 + $r2 + 1;
      #warn "length is : ". $ln;
      #warn "; centrepoint is:  ". $self->centrepoint();
...
sub seq {
  ...
  if($self->adaptor()) {
    ...
    if ($self->{'start'} > $self->{'end'}) {
        ...
        my $sl1= Bio::EnsEMBL::Slice->new(-COORD_SYSTEM               => $self->{'coord_system'},
    	...
    	my $sl2= Bio::EnsEMBL::Slice->new(-COORD_SYSTEM               => $self->{'coord_system'},
						  -SEQ_REGION_NAME    => $self->{'seq_region_name'},
						  -SEQ_REGION_LENGTH  => $self->{'seq_region_length'},
                                                  -START              => 1,
						  -END                => $self->{'end'},
        ...
        my $seq1 = ${$seqAdaptor->fetch_by_Slice_start_end_strand($sl1,1,undef,1)};
        my $seq2 = ${$seqAdaptor->fetch_by_Slice_start_end_strand($sl2,1,undef,1)};
       	return $seq1.$seq2;
      
    } else {
	my $seq1 = ${$seqAdaptor->fetch_by_Slice_start_end_strand($self,1,undef,1)};
	return $seq1;
    }
  }
...
#  }
  ...
  if($self->adaptor){
    ...
    my $seqAdaptor = $self->adaptor->db->get_SequenceAdaptor();
    if($end < $start ) {
       my $subseq1 = ${$seqAdaptor->fetch_by_Slice_start_end_strand
                 ( $self, $start,
                   $length, $strand )};
       my $subseq2 = ${$seqAdaptor->fetch_by_Slice_start_end_strand
                 ( $self, 1,
                   $end, $strand )};
...
sub project {
  ...
  my $sl01= Bio::EnsEMBL::CircularSlice->new(-COORD_SYSTEM      => $self->{'coord_system'},
                                            -SEQ_REGION_NAME    => $self->{'seq_region_name'},
                                            -SEQ_REGION_LENGTH  => $self->{'seq_region_length'},
                                            -START              => $self->{'start'},
                                            -END                => $self->{'seq_region_length'},
  ...
  my $sl02= Bio::EnsEMBL::CircularSlice->new(-COORD_SYSTEM      => $self->{'coord_system'},
                                            -SEQ_REGION_NAME    => $self->{'seq_region_name'},
                                            -SEQ_REGION_LENGTH  => $self->{'seq_region_length'},
                                            -START              => 1,
                                            -END                => $self->{'end'},
  ...
  foreach my $segment (@$normal_slice_proj) {
    ...
    foreach my $coord (@coords) {
      ...
      if($coord->isa('Bio::EnsEMBL::Mapper::Coordinate')) {
        ...
        # same coordinate system we were already in then we should just                                                                                                                                          
        # return the original region.  This can happen for example, if we                                                                                                                                        
        # were on a PAR region on Y which refered to X and a projection to                                                                                                                                               # 'toplevel' was requested.                                                                                                                                                                               
        if($coord_cs->equals($slice_cs)) {
        # trim off regions which are not defined                                                                                                                                                                
          return $self->_constrain_to_region();
        }
...
## circular BOF
  my $sl1= Bio::EnsEMBL::CircularSlice->new(-COORD_SYSTEM      => $self->{'coord_system'},
					    -SEQ_REGION_NAME    => $self->{'seq_region_name'},
					    -SEQ_REGION_LENGTH  => $self->{'seq_region_length'},
					    -START              => $self->{'start'},
					    -END                => $self->{'seq_region_length'},
  ...
  my $sl2= Bio::EnsEMBL::CircularSlice->new(-COORD_SYSTEM      => $self->{'coord_system'},
					    -SEQ_REGION_NAME    => $self->{'seq_region_name'},
					    -SEQ_REGION_LENGTH  => $self->{'seq_region_length'},
                                            -START              => 1,
					    -END                => $self->{'end'},
  ...
  if( defined $attrib_code ) {                                                                                                                                                                                   
    my @res1 = grep { uc($_->code()) eq uc($attrib_code) } 
                @{$attribute_adaptor->fetch_all_by_Slice( $sl1 )};
    my @res2 = grep { uc($_->code()) eq uc($attrib_code) }
...
## circular BOF
   ...
   my (@arr, @arr1, @arr2, @arr2f);
   @arr1 =  @{$pta->fetch_all_by_Slice($sl1, $logic_name, $load_exons)};
   @arr2 =  @{$pta->fetch_all_by_Slice($sl2, $logic_name, $load_exons)};

   foreach my $g (@arr2) {
     push @arr2f, $g unless ($g->seq_region_start() > $g->seq_region_end());
   }
...
## circular BOF                                                                                                                                       
   ...
   my %union;
   if(defined($score)){
       @arr1 = @{$dafa->fetch_all_by_Slice_and_score($sl1,$score, $logic_name)};
       @arr2 = @{$dafa->fetch_all_by_Slice_and_score($sl2,$score, $logic_name)};
       push @arr, @arr1, @arr2;
       return \@arr;
   } 
   @arr1 = @{$dafa->fetch_all_by_Slice_and_hcoverage($sl1,$hcoverage, $logic_name)};
   @arr2 = @{$dafa->fetch_all_by_Slice_and_hcoverage($sl2,$hcoverage, $logic_name)};
   push @arr, @arr1, @arr2;
   return \@arr;
...
sub get_all_ProteinAlignFeatures {
  ...
  my (@arr, @arr1, @arr2);
  if(defined($score)){
      @arr1 = @{$pafa->fetch_all_by_Slice_and_score($sl1, $score, $logic_name)};
      @arr2 = @{$pafa->fetch_all_by_Slice_and_score($sl2, $score, $logic_name)};          

      push @arr, @arr1, @arr2;
      return \@arr;
  }

  @arr1 = @{$pafa->fetch_all_by_Slice_and_hcoverage($sl1, $hcoverage, $logic_name)};
  @arr2 = @{$pafa->fetch_all_by_Slice_and_hcoverage($sl2, $hcoverage, $logic_name)};
...
## circular BOF                                                                                                                                                                                                   
  ...
  my (@arr, @arr1, @arr2, @arr2f);
  @arr1 = @{$ga->fetch_all_by_Slice($sl1, $logic_name, $load_transcripts, $source, $biotype)};
  @arr2 = @{$ga->fetch_all_by_Slice($sl2, $logic_name, $load_transcripts, $source, $biotype)};
  
  #Due to this piece of code the second half of each overlapping the ori feature will be missing in Alignments (image) page where the two parts of the slice are displayed separately  
  foreach my $g (@arr2) {
    push @arr2f, $g unless ($g->seq_region_start() > $g->seq_region_end());
  }
...
## circular BOF                                                                                                                                                                                                                                                                                                                                   
  ...
  my (@arr, @arr1, @arr2, @arr2f);
  @arr1 = @{$ta->fetch_all_by_Slice($sl1, $load_exons, $logic_name)};
  @arr2 = @{$ta->fetch_all_by_Slice($sl2, $load_exons, $logic_name)};
...
sub get_all_Exons {
  ...
  my (@arr, @arr1, @arr2, @arr2f);
  @arr1 = @{$sl1->adaptor->db->get_ExonAdaptor->fetch_all_by_Slice($sl1)};
  @arr2 = @{$sl2->adaptor->db->get_ExonAdaptor->fetch_all_by_Slice($sl2)};
...
sub get_all_MiscFeatures {
  ...
  if($misc_set) {
     @arr1 = @{$mfa->fetch_all_by_Slice_and_set_code($sl1,$misc_set)};
     @arr2 = @{$mfa->fetch_all_by_Slice_and_set_code($sl2,$misc_set)};
     push @arr, @arr1, @arr2;
     return \@arr;
  }
...
sub get_all_MarkerFeatures {
  ...
  my (@arr, @arr1, @arr2);
  @arr1 = @{$ma->fetch_all_by_Slice_and_priority($sl1, $priority, $map_weight, $logic_name)};
  @arr2 = @{$ma->fetch_all_by_Slice_and_priority($sl2, $priority, $map_weight, $logic_name)};
  push @arr, @arr1, @arr2;
  return \@arr;
  ## circular EOF                                    
...
sub get_all_compara_DnaAlignFeatures {
  ...
  my (@arr, @arr1, @arr2);
  @arr1 = @{$dafa->fetch_all_by_Slice($sl1, $qy_species, $qy_assembly, $alignment_type)};
  @arr2 = @{$dafa->fetch_all_by_Slice($sl2, $qy_species, $qy_assembly, $alignment_type)};
  push @arr, @arr1, @arr2;
  return \@arr;
  ## circular EOF          
...
sub get_all_compara_Syntenies {
  ...
  my (@arr, @arr1, @arr2);
  @arr1 = @{$sra->fetch_by_MethodLinkSpeciesSet_DnaFrag($mlss, $dnafrag, $sl1->start, $sl1->end)};
  @arr2 = @{$sra->fetch_by_MethodLinkSpeciesSet_DnaFrag($mlss, $dnafrag, $sl2->start, $sl2->end)};
  push @arr, @arr1, @arr2;
  return \@arr;
  ## circular EOF                  
